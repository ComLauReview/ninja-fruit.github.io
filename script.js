...
function renderScoreHud() {
    ...
    cubeCountNode.innerText = `Quả đã chém được: ${state.game.cubeCount}`;
}
...
function renderMenus() {
    ...
    switch (state.menus.active) {
        ...
        case MENU_SCORE:
            ...
            if (isNewHighScore()) {
                highScoreLblNode.textContent = 'Kỉ lục mới!';
            } else {
                highScoreLblNode.textContent = `Điểm Cao Nhất: ${formatNumber(getHighScore())}`;
            }
